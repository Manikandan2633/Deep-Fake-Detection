...
EXTRACTED_FRAMES_FOLDER = 'E:\Final year Project\DeepFake_Detection\static\extracted_frames'

SAVE_FRAME = 'E:/Final year Project/DeepFake_Detection/static/Frames'
...
def extract_frames(video_path, frame_count=20):
  """
  Extracts frames from a video and saves them in the specified folder.
  ...
  Returns:
      A list of filenames of the extracted frames.
  """
  frame_filenames = []
  ...
  while success and count < frame_count:
    ...
    if faces:
      # Extract frame with the first detected face
      top, right, bottom, left = faces[0]
      frame = image[top:bottom, left:right, :]
      filename = f"{name}_{count}.jpg"
      frame_path = os.path.join(EXTRACTED_FRAMES_FOLDER, filename)
      cv2.imwrite(frame_path, frame)
      frame_filenames.append(filename)  # Append only the filename
    count += 1
    success, image = vidcap.read()

  vidcap.release()
  return frame_filenames  # Return only the filenames


def extractFrame(video_path, frame_count=20):
  """
  Extracts full frames from a video and saves them in the specified folder.
  ...
  while success and count < frame_count:
    filename = f"{name}_{count}.jpg"
    frame_path = os.path.join(SAVE_FRAME, filename)
...
def DetectPage():
    ...
    if request.method == 'POST':
        ...
        print(os.path.join(app.config['UPLOAD_FOLDER'], video_filename))
        video.save(os.path.join(app.config['UPLOAD_FOLDER'], video_filename))
        video_path = "Uploaded_Files/" + video_filename
        # Extract frames and get prediction
        extracted_frames = extract_frames(video_path)
        Frames = extractFrame(video_path)
        ...
        # print(data.output)
        return render_template('index.html',output=output,confidence=confidence, extracted_frames=extracted_frames,Frames=Frames)
